...
runCheck('M4 pages are registered and wired through isolated entries', () => {
  ...
  assert.ok(contactPanelWxml.includes('class="contact-row"'));
  assert.ok(contactPanelWxml.includes('contact-value contact-value-action'));
  assert.ok(contactPanelWxml.includes('bindtap="handleShowWechatQr"'));
  assert.ok(!contactPanelWxml.includes('class="contact-action"'));
  assert.ok(!contactPanelWxml.includes('>复制</button>'));
  assert.ok(!contactPanelWxml.includes('>查看</button>'));
  assert.ok(!contactPanelWxml.includes('拨打电话'));
...
runCheck('resume preference settings are wired through home contact and tool panels', () => {
  ...
  assert.ok(!contactPanelWxml.includes('link-copy-button'));
